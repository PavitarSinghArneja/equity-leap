-- =============================================
-- ANALYTICS HELPER FUNCTIONS AND TRIGGERS (FIXED)
-- =============================================
...
BEGIN
    ...
    score := score + LEAST(days_active * 3, 25);
    score := score + LEAST(avg_session_duration / 120, 15);
    score := score + LEAST(total_page_views / 10, 10);
    score := score + LEAST(properties_viewed * 2, 15);
    score := score + LEAST(properties_saved * 3, 12);
    score := score + LEAST(properties_with_notes * 2, 8);
    score := score + LEAST(avg_time_per_property / 300, 5);
    score := score + LEAST(search_queries, 3);
    score := score + LEAST(filter_applications, 3);
    score := score + LEAST(document_downloads, 4);
...
BEGIN
    ...
    SELECT
        COUNT(CASE WHEN total_time_spent > 600 THEN 1 END),
    ...
    -- Research depth
    SELECT COUNT(CASE WHEN event_type = 'property_document_download' THEN 1 END)
    ...
    score := score + LEAST(investment_flows_started * 15, 30);
    score := score + LEAST(investment_flows_completed * 25, 25);
    score := score + LEAST(properties_with_long_views * 5, 15);
    score := score + LEAST(properties_with_notes * 3, 12);
    score := score + LEAST(document_downloads * 2, 8);
    score := score + LEAST(recent_activity_frequency * 2, 10);
    score := score + kyc_progress * 3;
...
BEGIN
    ...
    IF starts > 0 THEN
        investment_abandonment_rate := (starts - completions)::DECIMAL / starts;
    ELSE
        investment_abandonment_rate := 0;
    END IF;
...
-- Function for daily analytics aggregation
