-- Complete fix for ambiguous column references in instant_buy_shares
-- The issue is that RECORD variables contain fields with the same names as table columns
...
BEGIN
  ...
  SELECT id, shares_owned, price_per_share
  INTO v_seller_inv_id, v_seller_inv_shares, v_seller_inv_price
  FROM investments
  ...
  IF v_seller_inv_id IS NOT NULL THEN
    IF v_seller_inv_shares > p_shares THEN
      ...
      SET shares_owned = v_seller_inv_shares - p_shares,
          total_investment = GREATEST(total_investment - (p_shares * v_seller_inv_price), 0),
          updated_at = now()
