-- Fix ambiguous column reference in instant_buy_shares
...
BEGIN
  ...
  ELSE
    -- FIXED: Use explicit references to avoid ambiguous column names
    UPDATE investments
    SET shares_owned = investments.shares_owned + p_shares,
        total_investment = investments.total_investment + v_amount,
        price_per_share = (investments.total_investment + v_amount) / NULLIF(investments.shares_owned + p_shares, 0),
        updated_at = now()
  ...
  IF FOUND THEN
    IF v_seller_inv.shares_owned > p_shares THEN
      -- FIXED: Use explicit table reference
      UPDATE investments
      SET shares_owned = investments.shares_owned - p_shares,
          total_investment = GREATEST(investments.total_investment - (p_shares * v_seller_inv.price_per_share), 0),
          updated_at = now()
